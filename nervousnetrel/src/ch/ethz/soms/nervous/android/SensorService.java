...
import ch.ethz.soms.nervous.android.sensors.InformationSensor;
import ch.ethz.soms.nervous.android.sensors.InformationSensor.InformationListener;
...
public class SensorService extends Service implements SensorEventListener, NoiseListener, BatteryListener, BLEBeaconListener, ConnectivityListener, InformationListener {
	...
	private InformationSensor sensorInformation = null;
	...
	private SensorCollectStatus scInformation = null;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		scInformation = sensorConfiguration.getInitialSensorCollectStatus(SensorDescInformation.SENSOR_ID);
		...
		sensorInformation = new InformationSensor(getApplicationContext());
		...
		scheduleSensor(SensorDescInformation.SENSOR_ID);
	...
	private void scheduleSensor(final long sensorId) {
		...
		final Runnable run = new Runnable() {
			...
			public void run() {
				...
				}else if (sensorId == SensorDescInformation.SENSOR_ID) {
					scInformation.setMeasureStart(startTime);
					doCollect = scInformation.isCollect();
					if (doCollect) {
						sensorInformation.clearListeners();
						sensorInformation.addListener(sensorListenerClass);
						sensorInformation.start();
					}
					sensorCollectStatus = scInformation;
...
//					scBLEBeacon.setMeasureInterval(3000);
	...
	public void informationSensorDataReady( long timestamp,  float entropyX,  float entropyY, float entropyZ,
		...
		ArrayList<SensorDesc> sensorDescs = new ArrayList<SensorDesc>();
		SensorDesc sensorDesc = new SensorDescInformation(timestamp, entropyX, entropyY, entropyZ, isLogging, isSharing);
	...
	private void unregisterSensor(long sensorId) {		
		if (sensorId == SensorDescAccelerometer.SENSOR_ID) {
			...
			sensorInformation.removeListener(this);
		}
	}
